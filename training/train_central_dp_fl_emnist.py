...
class ClientDataset(Dataset):
    def __init__(self, subset):
        self.dataset = subset  # dataset is a torch.utils.data.Subset

    def __len__(self):
        return len(self.dataset)

    def __getitem__(self, idx):
        return self.dataset[idx]  # returns (x, y) already
...
for rnd in range(1, TOTAL_ROUNDS + 1):
    ...
    noised_grad = add_noise(avg_grad, NOISE_MULTIPLIER, device)
    for param, grad in zip(global_model.parameters(), noised_grad):
        param.data += grad
...
# ====== LOGGING & PLOTTING ======
with open(f"{LOG_DIR}/central_dp_metrics.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
plt.savefig(f"{LOG_DIR}/central_dp_privacy_vs_rounds.png")
plt.clf()

plt.plot(eps, accs)
plt.xlabel("Privacy Loss (ε)")
plt.ylabel("Accuracy")
plt.title("Central DP-FL: Accuracy vs Privacy")
plt.savefig(f"{LOG_DIR}/central_dp_privacy_vs_accuracy.png")
