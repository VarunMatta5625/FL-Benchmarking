...
def sample_gaussian(mean, std, shape, device):
    return mean + std * torch.randn(shape).to(device)
...
for rnd in range(1, TOTAL_ROUNDS + 1):
    ...
    for client_data in clients:
        ...
        posterior_grads.append(clipped)

    # Bayesian DP: Sample from posterior (mean + noise)
    mean_grads = [sum(p[i] for p in posterior_grads) / NUM_CLIENTS for i in range(len(posterior_grads[0]))]
    sampled_grads = [
        sample_gaussian(g, NOISE_MULTIPLIER * CLIP, g.shape, device)
        for g in mean_grads
    ]

    for param, grad in zip(model.parameters(), sampled_grads):
        param.data += grad
...
with open("logs/bayesian_dpfl_metrics.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
plt.xlabel("Rounds")
plt.ylabel("Accuracy")
plt.title("Bayesian DP-FL: Accuracy vs Rounds")
plt.savefig("logs/bayesian_dpfl_accuracy_vs_rounds.png")
plt.clf()

plt.plot(epsilons, accs)
plt.xlabel("Privacy Loss (ε)")
plt.ylabel("Accuracy")
plt.title("Bayesian DP-FL: Accuracy vs Privacy")
plt.savefig("logs/bayesian_dpfl_accuracy_vs_privacy.png")